...
public class UrnMetadata {
    ...
    public static String getAllStereotypes( URNmodelElement reference, URNmodelElement parent ) {
    	...
    	if( checkRootFeature(reference, parent)){
     ...
     public static boolean checkRootFeature(URNmodelElement reference, URNmodelElement parent){
    	boolean rootFeature = false;
    	for (Iterator it = parent.getMetadata().iterator();it.hasNext();){
    		 Metadata metadata = (Metadata) it.next();
    		 String name = metadata.getName();
    		 String value = metadata.getValue();
    		 if(name.compareTo("CoURN")==0  &&  value.compareToIgnoreCase("root feature")==0){
    			 rootFeature = true;
    		 }
    	}
