...
public class FeatureUtil {
	...
	/** 
	 * Returns true if elem is Feature and it is root feature in Concern-Oriented .jucm file
	 * @param Feature elem
	 * @return true or false
	 */
	public static boolean isRootFeatureCoURN(Feature elem){
		String value = MetadataHelper.getMetaData(elem, "CoURN");
    ...
    public static boolean checkFeatureXORErrorNoneSelected(Feature elem){
    	Iterator it = elem.getLinksSrc().iterator();
        // if it doesn't have source, return false
        if (!it.hasNext()) {
            return false;
        ...
        while (it.hasNext()) {
        	ElementLink link = (ElementLink) it.next();
        	IntentionalElement srcElem = (IntentionalElement)link.getDest();
        	boolean broElemSelected=false;	
        	if (srcElem instanceof Feature && link instanceof Decomposition) {
        		// if decomposition type is XOR and the relevant type needs to be checked
        		if ((srcElem.getDecompositionType() == DecompositionType.XOR_LITERAL)) {
        			...
        			while (srcIt.hasNext()) {
        				...
        				if (broLink instanceof Decomposition) {
        					...
        					IntentionalElement broElem = (IntentionalElement)broLink.getSrc();
        					if (broElem instanceof Feature && !(broElem.equals(elem)) && checkSelectionStatus((Feature) broElem, true)) {
        						broElemSelected = true;
        					}
        				}
        			}
        	    }
        	   if(checkSelectionStatus((Feature)srcElem,true) && !broElemSelected)
            		return true;	
        	}
        }
        return false;
    ...
    public static boolean checkFeatureORErrorNoneSelected(Feature elem){
    	Iterator it = elem.getLinksSrc().iterator();
        // if it doesn't have source, return false
        if (!it.hasNext()) {
            return false;
        ...
        while (it.hasNext()) {
        	...
        	boolean broElemSelected=false;
        	if (srcElem instanceof Feature && link instanceof Decomposition) {
        		// if decomposition type is  OR and the relevant type needs to be checked
