 ...
 * 
 * @author jkealey, gunterm, pchen,jiaying
 * 
 */
        ...
        if (createFmd) {
        	// add the root feature in the newly created feature diagram if it is CoURN
        	 if(createCRN){
        	      ...
        	      Object fture=getNewObject(urnspec,IntentionalElementRef.class,ModelCreationFactory.FEATURE);
        	      urnspec.getGrlspec().getIntElements().add(((IntentionalElementRef) fture).getDef()); 
        	      ((IntentionalElementRef)fture).setX(160);
        	      ((IntentionalElementRef)fture).setY(160);
        	      ((IntentionalElementRef)fture).getDef().setName(filename.substring(0,1).toUpperCase() + filename.substring(1));
        	      ((IntentionalElementRef)fture).setCriticality(null);
        	      
        	      // want to check what happened if the "CoURN/root feature" metadata is added to 
        	      // both IntentionalElementRef and IntentionalElement Def
        	      
        	      
        	      MetadataHelper.addMetaData(urnspec, ((IntentionalElementRef) fture).getDef(), "CoURN", "root feature");
//        	      mh.addMetaData(urnspec, (IntentionalElementRef) fture ,"CoURN", "root feature");
//        	      Metadata newMeta= (Metadata) getNewObject(urnspec,Metadata.class);
//        	      newMeta.setName("CoURN");
//        	      newMeta.setValue("root feature");
//        	      ((IntentionalElementRef) fture).getDef().getMetadata().add(newMeta);
     
        	      // ((IntentionalElementRef) fture).getMetadata().add(newMeta);
        	      fd.getNodes().add(((IntentionalElementRef) fture));
