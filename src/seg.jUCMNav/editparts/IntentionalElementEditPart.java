 ...
 * 
 * @author Jean-Francois Roy, sghanava, jkealey, gunterm, Jiaying
 * 
 */
public class IntentionalElementEditPart extends GrlNodeEditPart implements NodeEditPart {
    ...
    private void setText() {
        if (getNode().getDef() != null) {
            ...
            System.out.println("The name of the intentional elementref is "+ name);
            String value = MetadataHelper.getMetaData(getNode().getDef(), "CoURN");
            if (value!=null && value.equalsIgnoreCase("root feature") ){
            	
                   URNspec urn = getNode().getDef().getGrlspec().getUrnspec();
                   String valueURN= MetadataHelper.getMetaData(urn, "CoURN");
                   if (valueURN!=null && valueURN.equalsIgnoreCase("true") && urn.getUrndef().getConcerns().size()>0)
            	   {
                	   String rename=((Concern)urn.getUrndef().getConcerns().get(0)).getName();
                	   if (!name.equals(rename)){
                		  getNode().getDef().setName(rename);
                		  name= rename;
                	   } 
            	   } 	  
            }
